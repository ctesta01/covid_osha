...
load_osha <- function(export=T) {
  osha_closed <- openxlsx::read.xlsx(
    system.file(
      'raw_data/Copy_of_Closed_Federal_and_State_Plan_Valid_Covid-19_Complaints_through_August_23.xlsx',
  ...
  osha_open <- openxlsx::read.xlsx(
    system.file(
      'raw_data/Copy_of_Open_Federal_and_State_Plan_Valid_Covid-19_Complaints_through_August_23.xlsx',
      package = 'osha.analysis'
    )
  )
